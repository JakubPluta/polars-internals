...
def main():
    ...
    data: pl.LazyFrame = (
        ...
        .select("tpep_pickup_datetime", "VendorID", "payment_type", "total_amount")
        .with_columns(
        ...
        .group_by(pl.col("VendorID"), pl.col("payment_type"), pl.col("pickup_month"))
        .agg(total_amount_per_vendor_payment_type_month=pl.col("total_amount").sum())
    )

    # Sink to Parquet with optimized settings
    data.sink_parquet(
        NYC_YELLOW_TAXI_DIR / "nycagg.parquet",
        compression="snappy",
...
if __name__ == "__main__":
    ...
    Output Data Size: 9kb - 1 Parquet File, 4 Columns, 25 Rows

    Processing time:
        Time: 7 seconds
        Maximum memory used: 316.9 MiB
    """
    main()
